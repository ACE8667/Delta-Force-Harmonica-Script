...
function playNote(noteIndex) {
  ...
  if (running) {
    const candidate = tiles.filter((tile) => tile.noteIndex === noteIndex)
      .filter((tile) => isTileInHitZone(tile))
      .sort((a, b) => Math.abs(a.y - b.y))[0];
    if (candidate) hitTile(candidate);
  }
}
...
function updateHitFeedback(tile) {
  ...
  if (ready) hitZone.classList.add("ready");
}
...
document.addEventListener("keydown", (event) => {
  if (event.code === "Space") { event.preventDefault(); startGame(); return; }
  const noteIndex = NOTES.findIndex((note) => note.key.toLowerCase() === event.key.toLowerCase());
  if (noteIndex >= 0 && !event.repeat) playNote(noteIndex);
});
